// CimMacro_tc.sv
// Ablation baseline: identical to CimMacro, but replaces the SM DualAdderTree
// with a 2's-complement single adder tree (DualAdderTree_TC).
//
// Architecture delta vs CimMacro:
//   - DualAdderTree  → DualAdderTree_TC
//   - Everything else (LUT, ZpSplitter, ZpCompensate, OutReg) is identical
//
// Purpose: quantify the timing/area/power cost of SM routing vs TC conversion.
...
module CimMacro_tc #(
    ...
    // -----------------------------------------------------------------------
    // TC Single Adder Tree (ablation: replaces SM DualAdderTree)
    // -----------------------------------------------------------------------
    logic signed [31:0] raw_result;

    DualAdderTree_TC #(.N(N_GROUPS)) uDualTree (
        .sm_in  (sm_in),
        .result (raw_result)
    );

    // -----------------------------------------------------------------------
    // ZP Compensation (identical to CimMacro)
    // -----------------------------------------------------------------------

...
def main():
    # Two independent RNGs seeded from SEED:
    #   act_rng    — activations only (state varies with H4_MATCH_FRAC)
    #   weight_rng — weights only (state is always identical for the same SEED,
    #                regardless of H4/SIGMA/other activation params)
    ...
    # 1. Activations
    acts = gen_activations_sparse(act_rng)
    ...
    # 2. Weights
    rows = gen_weight_rows_sparse(weight_rng)
